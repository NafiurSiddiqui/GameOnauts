...
let cardContainer = document.querySelector('.game__container_game-link');
let gameCardContainer = document.querySelector('.game__container');
console.log( cardContainer);

// cardContainer.addEventListener('click',function(){console.log(`ok!`)})
...
const showGame = async function(){ 
    try{
      ...
      //3.Ajax call
        ...
        const markUp = results.forEach((game) =>{
        let id = game.id;
    ...
    <div class="game__container" data-id="${id}">
          <a href="javascript:;" class="game__container_game-link" data-link=""></a> 
    ...
    selectNode[i].addEventListener('click',check);
    }
   return selectNode;
...
// results.forEach(el => console.log(el.id));
